...
from watermark import display, mask_color, canny, mark_direction, quantize
...
def test_points(img, points):
    for name, p in points.items():
        ...
        for alpha in np.arange(0.2, 0.8, 0.1):
            ...
            print(f'alpha: {format(alpha, ".2f")}, mask: {mask}, std: {format(np.std(mask), ".2f")}')
...
def test_fruits():
    img = prepare_test_image('fruits.jpg')
...
def test_watermark():
    img = prepare_test_image('watermark.jpg')
    points = {
...
def test_watermark2():
    img = prepare_test_image('watermark2.jpg')
    points = {
        'a': np.array([[70, 58], [86, 38]]),
...
def test_mark_directions():
    img = cv2.imread('watermark2.jpg', -1)
